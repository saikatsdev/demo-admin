...
export default function OrderTag() {
    //Hook
    useTitle("All Order Tag List");

    // State
    const [query, setQuery]               = useState("");
    const [loading, setLoading]           = useState(false);
    const [orderTags, setItems]                = useState([]);
    const [isModalOpen, setIsModalOpen]   = useState(false);
    const [messageApi, contextHolder]     = message.useMessage();
    const [editingItems, setEditingItems] = useState(null);
    const [filteredData, setFilteredData] = useState(orderTags);
    ...
    useEffect(() => {
        if(!query){
            setFilteredData(orderTags);
        }

        const lowerQuery = query.toLowerCase();

        const filtered = orderTags?.filter(item => 
    ...
    useEffect(() => {
        ...
        const fetchContactList = async () => {
            ...
            const res = await getDatas("/admin/order-froms");
    ...
    const onDelete = async (id) => {
        const res = await deleteData(`/admin/order-froms/${id}`);
        ...
        <>
            ...
            <div className="pagehead">
                <div className="head-left">
                    <h1 className="title">All Order Tag List</h1>
                </div>
                <div className="head-actions">
                    <Breadcrumb
                        items={[
                            { title: <Link to="/dashboard">Dashboard</Link> },
                            { title: "All Order Tag List" },
                        ]}
                    />
                </div>
            </div>
