...
export default function FraudCheck() {
    ...
    const totals = useMemo(() => {
        const totalOrders = courierList.reduce((sum, i) => sum + Number(i.total_parcels || 0),0);

        const totalDelivered = courierList.reduce((sum, i) => sum + Number(i.delivered_parcels || 0),0);
                ...
                {!courierData && !loading ? (
                    <div className="fc-empty-wrap">
                        <div className="fc-empty-card">
                            ...
                            <Form form={form} onFinish={handleCheck} className="fc-search-form fc-search-form--center">
                                <Form.Item name="phone_number" rules={[{ required: true, message: "Please enter phone number" }, {pattern: /^[0-9]+$/,message: "Only numbers are allowed"}]}>
                                    <Input placeholder="Mobile Number" allowClear className="fc-input" size="large"/>
                                </Form.Item>

                                <Button type="primary" htmlType="submit" loading={loading} className="fc-btn">
                                    রিপোর্ট দেখুন
                                </Button>
                            </Form>
                    ...
                    <div className="fc-board">
                        <div className="fc-card fc-card--left">
                            ...
                            <div className="fc-circle-wrap">
                                <Progress type="circle" percent={Math.min(100, Math.max(0, totals.overallSuccess))} strokeColor={circleStroke} trailColor={circleTrail} size={268} strokeWidth={12} format={() => ""}/>
                            ...
                            {!hasOrders ? (
                                ...
                                <>
                                    <div className={`fc-risk-text ${isSafe ? "is-safe" : "is-risk"}`}>
                                        {isSafe ? "ঝুঁকি মুক্ত" : "ঝুঁকিপূর্ণ"}
                                    </div>

                                    <div className="fc-risk-sub">
                                        {isSafe ? "এই কাস্টমারকে নিশ্চিন্তে পার্সেল দিতে পারেন।" : "এই কাস্টমারের সাথে ডেলিভারিতে সতর্ক থাকুন।"}
                                    </div>
                                </>
                            )}
                        </div>
                        ...
                        <div className="fc-card fc-card--right">
                            <div className="fc-topbar">
                                <Form form={form} onFinish={handleCheck} className="fc-search-form fc-search-form--right">
                                    <Form.Item name="phone_number" rules={[{required: true, message: "Please enter phone number" }, {pattern: /^[0-9]+$/,message: "Only numbers are allowed"}]}>
                                        <Input placeholder="Mobile Number" allowClear className="fc-input" size="large"/>
                                    </Form.Item>
                            ...
                            <div className="fc-table-card">
                                ...
                                <div className="fc-ct-body">
                                    {rowsWithTotal.map((r) => {
                                        ...
                                        return (
                                            <div key={`${r.courier_name}-${isTotal ? "t" : "n"}`} className={`fc-ct-row ${r.rowClass || ""}`}>
                                                <div className="fc-ct-courier">
                                                    {isTotal ? (
                                                        <div className="fc-total-text">Total</div>
                                                    ) : (
                                                        <div className="fc-logo-pill" title={r.courier_name}>
                                                            <img src={r.logo} alt={r.courier_name} className="fc-logo"/>
                                                        </div>
                                                    )}
                                                </div>
