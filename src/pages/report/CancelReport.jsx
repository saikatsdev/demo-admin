...
import { FilePdfOutlined, FileExcelOutlined, ReloadOutlined, ArrowLeftOutlined, PrinterOutlined, CalendarOutlined, SearchOutlined, UserOutlined, ShoppingOutlined, StopOutlined, WarningOutlined, DollarOutlined, BarChartOutlined, LineChartOutlined,CloseCircleOutlined} from "@ant-design/icons";
...
export default function CancelReport() {
    // Hooks
    useTitle("Cancel Order Performance Analytics");
    ...
    const getCancelReport = async () => {
        ...
        try {
            ...
            if (res && res?.success) {
                ...
                setTopProducts(result?.top_canceled_products || []);
                setPagination(prev => ({ 
                    ...prev, 
    ...
    const downloadPDF = () => {
        ...
        doc.setFontSize(16);
        doc.text("Canceled Order Performance & Reason Analytics", 14, 20);
        doc.setFontSize(10);
        doc.text(`Generated on: ${dayjs().format("YYYY-MM-DD HH:mm")}`, 14, 27);
        
        const tableColumn = ["#", "Customer", "Phone", "Invoice", "Reason", "District", "Canceled Products", "Canceled Value", "Payable", "Date"];
        const tableRows = dataToExport.map((o, i) => {
            const productNames = o.cancel_details?.map(d => `${d.product_name} (x${d.quantity})`).join(', ') || 'N/A';
            return [
                i + 1,
                o.customer_name,
                o.phone_number,
                o.invoice_number,
                o.cancel_reason_info?.name || "Unspecified",
                o.district?.name || "N/A",
                productNames,
                `৳${Number(o.cancel_summary?.total_value || o.net_order_price || 0).toLocaleString()}`,
    ...
    const downloadCSV = () => {
        const dataToExport = getExportData();
        const headers = ["SL", "Invoice", "Customer Name", "Phone Number", "Cancel Reason", "Order Source", "Courier", "District", "Paid Status", "Canceled Items List", "Canceled Items Count", "Canceled Quantity", "Canceled Value", "Delivery Charge", "Advance Payment", "Net Price", "Payable Price", "Date"];
        const rows = dataToExport.map((o, i) => {
            const productNames = o.cancel_details?.map(d => `${d.product_name} (Qty: ${d.quantity}, Total: ৳${d.line_total})`).join('; ') || '';
            const s = o.cancel_summary || {};
            return [
                i + 1,
                `"${o.invoice_number || ''}"`,
                `"${o.customer_name || ''}"`,
    ...
    const columns = [
            ...
            render: (_, record) => (
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <Avatar 
                        size={38} 
                        icon={<UserOutlined />} 
                        style={{ backgroundColor: '#fef2f2', color: '#ef4444', border: '1px solid #fecaca', flexShrink: 0 }} 
                    />
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                    ...
                    <Space size={4} wrap>
                        {record.current_status && (
                            <Tag 
                                style={{ 
                                    backgroundColor: (record.current_status.bg_color || '#ef4444') + '18', 
                                    color: record.current_status.bg_color || '#dc2626',
                                    border: `1px solid ${(record.current_status.bg_color || '#fecaca')}50`,
                                    borderRadius: 4,
                                    fontWeight: 700,
                                    fontSize: 9.5,
                                    margin: 0,
                        ...
                        {record.courier && (
                            <Tag color="volcano" style={{ fontSize: 9.5, margin: 0, borderRadius: 4, fontWeight: 600, padding: '0 4px' }}>
                                {record.courier.name}
                            </Tag>
                        )}
                    </Space>
        ...
        },
        {
            ...
            render: (_, record) => (
                ...
                >
                    {record.cancel_reason_info?.name || "Unspecified Reason"}
                </Tag>
            ),
            width: 165
        },
        {
            ...
            key: "canceled_items",
            render: (_, record) => {
                const details = record.cancel_details || [];
                if (details.length === 0) {
                        ...
                        {details.map((item, idx) => (
                            <div key={item.id || idx} style={{ 
                                ...
                                <Avatar 
                                    ...
                                    style={{ borderRadius: 4, flexShrink: 0, border: '1px solid #fca5a5', background: '#fff' }} 
                                />
                                <div style={{ display: 'flex', flexDirection: 'column', minWidth: 0, flex: 1 }}>
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 9.5, marginTop: 1 }}>
                                        <Tag color="red" style={{ fontSize: 9, margin: 0, padding: '0 4px', borderRadius: 3, fontWeight: 700 }}>
                                            x{item.quantity} Pcs
                                        </Tag>
                                ...
                                <div style={{ textAlign: 'right', flexShrink: 0 }}>
                                    <div style={{ fontSize: 11.5, fontWeight: 700, color: Number(item.line_total || 0) >= 0 ? '#be123c' : '#dc2626' }}>
        ...
        },
        {
            ...
            align: 'center',
            render: (_, record) => {
                const s = record.cancel_summary || {};
                const val = Number(s.total_value ?? record.net_order_price ?? 0);

                return (
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                        <Tag color="volcano" style={{ margin: 0, borderRadius: 4, fontWeight: 700, fontSize: 10.5, padding: '1px 6px' }}>
                            {s.items_count || record.cancel_details?.length || 0} Items ({s.total_quantity || 0} Pcs)
                        </Tag>

                        <div style={{ background: '#fef2f2', padding: '3px 10px', borderRadius: 6, border: '1px solid #fecaca', display: 'inline-block' }}>
    ...
    return (
        <div className="reportWrapper">
            ...
            <div className="topBar no-print" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                <div>
                    <Title level={4} style={{ margin: 0, color: '#0f172a' }}>Cancel Order Performance Intelligence</Title>
                    <Text type="secondary" style={{ fontSize: 12 }}>Canceled order volume, attrition reasons, disruption financial values, and product cancellation metrics</Text>
                </div>
                <Space>
                    <Button icon={<ReloadOutlined />} onClick={getCancelReport} loading={loading}>
                        Refresh Data
                    </Button>
                ...
                <div className="no-print" style={{ marginBottom: 20 }}>
                    <Row gutter={[12, 12]}>
                        <Col xs={24} sm={12} md={6}>
                            <Card bordered={false} className="mini-summary-card red">
                                <div className="mini-card-top">
                                    <div className="mini-card-icon red">
                                        <CloseCircleOutlined />
                                    </div>
                                    <div className="mini-card-info">
                                        <span className="mini-card-label">Total Canceled Value</span>
                                        <span className="mini-card-val">
                                            ৳{Number(summary.total_canceled_value || 0).toLocaleString()}
                                        </span>
                                    </div>
                                </div>
                                <div className="mini-card-footer">
                                    <span className="footer-pill red">Payable: ৳{Number(summary.total_payable_price || 0).toLocaleString()}</span>
                        ...
                        <Col xs={24} sm={12} md={6}>
                            <Card bordered={false} className="mini-summary-card orange">
                                <div className="mini-card-top">
                                    ...
                                    <div className="mini-card-info">
                                        <span className="mini-card-label">Canceled Quantity & Products</span>
                                        <span className="mini-card-val">
                                            {Number(summary.total_canceled_quantity || 0).toLocaleString()} QTY
                                        </span>
                                    </div>
                                </div>
                                <div className="mini-card-footer">
                                    <span className="footer-pill orange">{summary.total_canceled_items || 0} Products</span>
                        ...
                        <Col xs={24} sm={12} md={6}>
                            <Card bordered={false} className="mini-summary-card purple">
                                <div className="mini-card-top">
                                    ...
                                    <div className="mini-card-info">
                                        <span className="mini-card-label">Reason Categorization</span>
                                        <span className="mini-card-val">
                                            {summary.orders_with_reason || 0} Categorized
                                        </span>
                                    </div>
                                </div>
                                <div className="mini-card-footer">
                                    <span className="footer-pill purple">{reasonBreakdown.length || 0} Reason Types</span>
                                </div>
                            </Card>
                        </Col>
                        ...
                        <Col xs={24} sm={12} md={6}>
                            <Card bordered={false} className="mini-summary-card green">
                                <div className="mini-card-top">
                                    ...
                                    <div className="mini-card-info">
                                        <span className="mini-card-label">Average Canceled Value</span>
                                        <span className="mini-card-val">
                                            ৳{Number(summary.average_canceled_value || 0).toLocaleString()}
                                        </span>
                                    </div>
                                </div>
                                <div className="mini-card-footer">
                                    <span className="footer-pill red">Disc: -৳{Number(summary.total_canceled_discount || 0).toLocaleString()}</span>
                                </div>
                            </Card>
                        </Col>
                    </Row>

                    {/* Top Canceled Products Bar */}
                    {topProducts.length > 0 && (
                        <Card size="small" style={{ marginTop: 12, borderRadius: 8, background: '#fafafa', border: '1px solid #f0f0f0' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 14, overflowX: 'auto', paddingBottom: 2 }}>
                                ...
                                {topProducts.map((product, idx) => (
                                    <Tooltip key={idx} title={`${product.product_name} — ${product.canceled_order_count} canceled orders (${product.total_canceled_quantity} pcs)`}>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: 6, whiteSpace: 'nowrap', background: '#fff', padding: '3px 8px', borderRadius: 6, border: '1px solid #e2e8f0' }}>
                                            ...
                                            <Tag color="red" style={{ margin: 0, borderRadius: 3, fontSize: 9.5, fontWeight: 700 }}>
                                                {product.total_canceled_quantity} pcs
                                            </Tag>
                                            <span style={{ fontSize: 10, color: '#dc2626', fontWeight: 600 }}>৳{Number(product.total_canceled_value || 0).toLocaleString()}</span>
                                        </div>
                                    </Tooltip>
                                ))}
                            </div>
