...
export default function CrossSellReport() {
    ...
    useEffect(() => {
        const getOrderReport = async () => {
            const res = await getDatas("/admin/order/reports");
    ...
    const filteredData = orders.filter(
        (item) => (item.phone_number.toLowerCase().includes(filter.toLowerCase()) || item.current_status.name.toLowerCase().includes(filter.toLowerCase()) ||
            item?.order_from?.name.toLowerCase().includes(filter.toLowerCase()))
    );
    ...
    return (
        <>
            <div className="reportWrapper">
                <h5 className="mb-4">Cross Sell Report</h5>
                        ...
                        <tbody>
                            {filteredData.map((item) => (
                                <tr key={item.id}>
                                    ...
                                    <td>
                                        <span className={`order-paid-btn ${item.paid_status === "paid" ? "paid" : "unpaid"}`}>
                                            {item.paid_status}
                                        </span>
                                    </td>
                                    <td>
                                        <span className={`statusBadge ${item.current_status.name.toLowerCase()}`}>
                                    ...
                                    <td>
                                        <span
                                            ...
                                            item?.order_from?.name.toLowerCase() === "Frontend" ? "source-facebook" : item.order_from?.name.toLowerCase() === "youtube"
                                                ? "source-youtube" : item?.order_from?.name.toLowerCase() === "frontend" ? "source-frontend" : "source-default"
                                            }`}
                                        >
