...
const extractPermIds = (roleOrPayload) => {
    ...
    const perms = roleOrPayload?.permissions?.data ?? roleOrPayload?.permissions ?? roleOrPayload?.data?.permissions?.data ?? [];
    ...
    const saveRole = async () => {
        try {
            ...
            if (res?.success) {
                messageApi.open({
                    type: "success",
                    content: editingRole ? 'Role updated' : 'Role created'
                });
            ...
            <>
                <div className="role-bar">
                    <Space wrap>
                        ...
                        <Checkbox style={{ marginLeft: 8 }} indeterminate={allPermissionIds.some(id => selectedPermissionIds.has(Number(id))) && !allPermissionIds.every(id => selectedPermissionIds.has(Number(id)))} checked={allPermissionIds.length > 0 && allPermissionIds.every(id => selectedPermissionIds.has(Number(id)))} onChange={(e) => toggleIds(allPermissionIds, e.target.checked)}>
                            Select all
                        </Checkbox>
                    </Space>

                    <Space>
                        <AntInput allowClear prefix={<SearchOutlined />} placeholder="Filter resources..." value={filterQ} onChange={(e) => setFilterQ(e.target.value)} style={{ width: 320 }}/>
                ...
                <div className="section-wrap">
                    ...
                    {filteredTree.map((sec) => {
                        ...
                        const sectionInd = sec.ids.some(id => selectedPermissionIds.has(Number(id))) && !sectionChecked;
                        return (
                            <div key={sec.section} className="section-block">
                                <div className="section-head">
                                    <Checkbox indeterminate={sectionInd} checked={sectionChecked} onChange={(e) => toggleIds(sec.ids, e.target.checked)}/>
                                        ...
                                        return (
                                            <div key={sec.section + '-' + it.resource} className="perm-card">
                                                <div className="perm-card-head">
                                                    <Checkbox indeterminate={rowInd} checked={rowChecked} onChange={(e) => toggleIds(it.ids, e.target.checked)}/>
                                                    <span className="perm-title">{it.resource}</span>
                                                </div>
                                                <div className="perm-actions">
                                                    {it.actions.map((a) => (
                                                        <label key={a.id} className="action-item">
                                                            <Checkbox checked={selectedPermissionIds.has(Number(a.id))} onChange={(e) => toggleIds([Number(a.id)], e.target.checked)}/>
