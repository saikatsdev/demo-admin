...
const ProductEdit = () => {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (res?.success) {
        ...
        //  Load sub-categories FIRST before setting the ID
        if (p.category?.id) {
          try {
            const subRes = await getDatas("/admin/sub-categories", {category_id: p.category?.id});
        ...
        //  Load sub-sub-categories FIRST before setting the ID
        ...
        //  Now set the IDs after the options are loaded
        setProductId(p.id || "");
        ...
        //  Keep product for variation processing
  ...
  //  Load variations after both attributes and product are available
  useEffect(() => {
    if (
      allVariations?.data?.length > 0 &&
      ...
      //  Fix: Load attribute IDs and values properly
      ...
      const attrData = p.variations.map((v) => {
        ...
        if (v.attribute_value_2) {
          const attr2 = allVariations.data.find(
            (a) => a.id === v.attribute_value_2.attribute_id
          );
          if (attr2) attrs.push(attr2);
        }
        if (v.attribute_value_3) {
          const attr3 = allVariations.data.find(
            (a) => a.id === v.attribute_value_3.attribute_id
          );
  ...
  const addVariationField = () => {
    //  Fix: Add new variation at the top (beginning of array)
    setDynamicInputs((prev) => ["", ...prev]);
    // Initialize all variation-related arrays for the new index at the top
    ...
    //  Clear normal pricing when switching to variations
    setBuyPrice("");
    setRegularPrice("");
    setOfferPrice("");

    //  Clear bulk selection when adding new variation
  ...
  const removeVariationField = (index) => {
    ...
    //  If no variations left, show normal pricing again
  ...
  const handleAttributeValue = (index, selectedIds = null) => {
    ...
    const newAttributes =
      allVariations?.data?.filter((i) => attributeIds.includes(i.id)) || [];

    let updated = [...attributes];
    updated[index] = newAttributes;
    setAttributes(updated);

    //  Fix: Only reset values for removed attributes, keep existing values for remaining attributes
    let attrVal = [...attributeValue];
    if (!attrVal[index]) attrVal[index] = [];

    // Keep only values that belong to still-selected attributes
    const currentValues = attrVal[index] || [];
    const filteredValues = currentValues.filter((val) =>
      attributeIds.includes(val.attribute_id)
    );
  ...
  const handleVariationMultiplySubmit = () => {
    ...
    //  Fix: Group selected values by attribute_id
    const groupedValues = {};
    ...
    //  Fix: Create combinations using only selected values
    ...
    // নতুন variation row বানানো (top এ add করব)
    flattened.forEach((combo) => {
      //  Fix: Add new variations at the top
      setDynamicInputs((prev) => ["", ...prev]);
      ...
      //  Fix: Set selected attribute IDs properly (at top)
      setSelectedAttributeId((prev) => [
        combo.map((c) => c.attribute_id),
        ...prev,
      ]);

      //  Fix: Set attribute values properly (at top)
      setAttributeValue((prev) => [combo, ...prev]);

      //  Fix: Set attributes for proper dropdown display (at top)
    ...
    //  Clear bulk selection when adding new variations
    setSelectedVariationRows([]);
    setSelectAllVariations(false);
  };
    ...
    try {
      ...
      const uniqueAttributes = [
        ...new Set(
          attributeValue
            .flat()
            .filter((x) => x?.attribute_id)
            .map((x) => x.attribute_id)
        ),
      ];

      const attributeSlotMap = {};
      uniqueAttributes.slice(0, 3).forEach((attrId, idx) => {
        attributeSlotMap[attrId] = idx + 1;
      });
    ...
    <div style={{ padding: 16 }}>
              ...
              <Row gutter={12}>
                  ...
                  <Col span={8}>
                    <Form.Item label="Sub Sub Category">
                      <Select
                        ...
                        value={subSubCategoryId || undefined}
                        loading={subSubCategoryLoading}
                        onChange={(v) => setSubSubCategoryId(v)}
                        options={(subSubCategories || []).map((ssc) => ({
                          label: ssc.name,
                          value: ssc.id,
                        }))}
                      />
                ...
                <Col span={8}>
                  <Form.Item label="Product Type">
                    <Select
                      ...
                      options={(productTypes?.data || []).map((t) => ({
                        label: t.name,
                        value: t.id,
                      }))}
                    />
                ...
                <Col span={8}>
                  <Form.Item label="Sold Quantity">
                    <Input value={soldQty} onChange={(e) => setSoldQty(e.target.value)} placeholder="0" status={errors?.name ? "error" : ""}/>
                  </Form.Item>
                </Col>
                  ...
                  <Col span={8}>
                    <Form.Item label="Current Stock">
                      <Input
                        ...
                        disabled={!isStockMaintainWithDirect}
                      />
                    </Form.Item>
                  </Col>
                )}
            ...
            <Space direction="vertical" size={16} style={{ width: "100%" }}>
                ...
                <label htmlFor="thumbnail-upload">
                  <div
                    style={{
                      ...
                      background: "#fafafa",
                      transition: "all 0.3s",
                      borderColor: errors?.image ? "#ff4d4f" : "#d9d9d9",
                    }}
                  >
                    <InboxOutlined
                      style={{
              ...
              <div>
                ...
                <Upload
                  ...
                  showUploadList={false}
                  beforeUpload={() => false}
                  onChange={handleReviewImageFileChange}
                  accept="image/*"
                >
                  <Button type="dashed" style={{ width: "100%" }}>
                    Choose Files
      ...
      <Row gutter={16}>
        <Col xs={24} lg={24}>
          <Card
            id="pricing-design-a"
            title="Product Price And Product Variation"
            bordered
          >
            <Row justify="space-between" style={{ marginBottom: 12 }}>
              <Col>
                {/*  Always show Add/Multiply buttons */}
                <Space>
                  <Button
                    type="default"
                    icon={<PlusOutlined />}
                  ...
                  <Button
                    type="default"
                    icon={<PlusOutlined />}
                    onClick={() => setVariationMultiplyModal(true)}
                  >
                    Variation Multiply
                  </Button>
                  {/*  Only show bulk delete when variations exist */}
                  {dynamicInputs.length > 0 && (
                    <Button
                      danger
                      disabled={selectedVariationRows.length === 0}
                      onClick={handleBulkDeleteVariations}
                    >
              ...
              <Col>
                {/*  Only show Select All when variations exist */}
                {dynamicInputs.length > 0 && (
                  <Space>
                    <input
                      type="checkbox"
                      ...
                      onChange={(e) =>
                        handleSelectAllVariations(e.target.checked)
                      }
                      style={{ marginRight: 8 }}
                    />
            ...
            {dynamicInputs.length > 0 ? (
              <div style={{ overflowX: "auto" }}>
                <Table
                  ...
                  pagination={false}
                  scroll={{ x: true }}
                  dataSource={dynamicInputs.map((_, index) => ({
                    key: index,
                    index,
                  }))}
                  columns={[
                    {
                      ...
                      render: (_, record) => (
                        <input
                          type="checkbox"
                          checked={selectedVariationRows.includes(record.index)}
                          onChange={(e) =>
                            handleSelectVariationRow(
                              record.index,
                              e.target.checked
                            )
                          }
                      ...
                      render: (_, record) => (
                        <Space direction="vertical" style={{ width: "100%" }}>
                          <Select
                            ...
                            style={{ width: "100%" }}
                            value={selectedAttributeId[record.index] || []}
                            onChange={(val) => {
                            ...
                            options={(allVariations?.data || []).map((v) => ({
                              label: v.name,
                              value: v.id,
                            }))}
                          />
                          {attributes[record.index]?.map((attr, i) => {
                            ...
                            return (
                              <Select
                                ...
                                style={{ width: "100%" }}
                                value={currentValue?.id || undefined}
                                onChange={(val) => {
                                  let updated = [...attributeValue];
                                  if (!updated[record.index])
                                    updated[record.index] = [];

                                  //  Fix: Remove existing value for this attribute first, then add new one
                                  updated[record.index] = updated[
                                    record.index
                                  ].filter((v) => v.attribute_id !== attr.id);

                                  // Find the full object from the selected value
                    ...
                    },
                    {
                      ...
                      render: (_, record) => (
                        <Input
                          placeholder="0"
                          value={variationBuyPrice[record.index]}
                          onChange={(e) =>
                    ...
                    },
                    {
                      ...
                      render: (_, record) => (
                        <Input
                          placeholder="0"
                          value={variationRegularPrice[record.index]}
                          onChange={(e) =>
                            updateVariationField(
                              record.index,
                              "regularPrice",
                    ...
                    },
                    {
                      ...
                      render: (_, record) => (
                        <Input
                          placeholder="0"
                          value={variationOfferPrice[record.index]}
                          onChange={(e) =>
                            updateVariationField(
                              record.index,
                              "offerPrice",
                    ...
                    },
                    {
                      ...
                      render: (_, record) => (
                        <Input
                          placeholder="1"
                          value={itemCurrentStock[record.index]}
                          onChange={(e) =>
                            updateVariationField(
                              record.index,
                              "stock",
                    ...
                    },
                    {
                      ...
                      render: (_, record) => (
                        <Select
                          placeholder="No"
                          value={isDefault[record.index]}
                          onChange={(val) =>
                            updateVariationField(record.index, "default", val)
                          }
                          options={[
                            { label: "No", value: 0 },
                            { label: "Yes", value: 1 },
                      ...
                      render: (_, record) => (
                        <Space direction="vertical" style={{ width: "100%" }}>
                          <input
                            ...
                            accept="image/*"
                            onChange={(e) =>
                              handleColorImageChange(e, record.index)
                            }
                    ...
                    },
                    {
                      ...
                      render: (_, record) => (
                        <Input.TextArea
                          ...
                          rows={2}
                          value={variationDescription[record.index]}
                          onChange={(e) =>
                            updateVariationField(
                              record.index,
                              "description",
                    ...
                    },
                    {
                      ...
                      render: (_, record) => (
                        <Button
                          danger
                          size="small"
                          onClick={() => removeVariationField(record.index)}
                        >
                          Delete
                        </Button>
                      ),
      ...
      {/*  Variation Multiply Modal */}
      <Modal
        title="Please Choose Your Variations"
        open={variationMultiplyModal}
        onCancel={() => setVariationMultiplyModal(false)}
        onOk={handleVariationMultiplySubmit}
        okText="Submit"
        width={800}
      >
        <Form layout="vertical">
          <Form.Item label="Variations">
            <Select
              ...
              onChange={(value) => {
                ...
                //  Fix: Only clear values for removed attributes, keep existing values
                const removedAttributeIds = selectedVariationId.filter(
                  (id) => !value.includes(id)
                );
                if (removedAttributeIds.length > 0) {
                  setSelectedVariationValues((prev) =>
                    prev.filter(
                      (val) => !removedAttributeIds.includes(val.attribute_id)
                    )
                  );
                }
              }}
          ...
          {/*  Show individual value selection for each selected attribute */}
          {selectedVariationId.map((attrId) => {
            const attribute = (allVariations?.data || []).find(
              (v) => v.id === attrId
            );
              ...
              <Form.Item key={attrId} label={`Select ${attribute.name} Values`}>
                <Select
                  ...
                  onChange={(valueIds) => {
                    ...
                    const otherValues = selectedVariationValues.filter(
                      (v) => v.attribute_id !== attrId
                    );

                    // Add new selected values for this attribute
                    const newValues = (attribute.values || []).filter((v) =>
          ...
          <div style={{ marginBottom: 12 }}>
            ...
            {selectedVariationValues.map((tag) => (
              <Tag
                ...
                closable
                onClose={() => {
                  //  Remove individual value from selectedVariationValues
                  setSelectedVariationValues((prev) =>
                    prev.filter((v) => v.id !== tag.id)
                  );
                }}
          ...
          {selectedVariationValues.length > 0 && (
            ...
            >
              ...
              {(() => {
                ...
                if (attributeGroups.length === 3) {
                  combinations = attributeGroups[0].flatMap((v1) =>
                    attributeGroups[1].flatMap((v2) =>
                      attributeGroups[2].map((v3) => [
                        v1.value,
                        v2.value,
                        v3.value,
                      ])
                  ...
                  combinations = attributeGroups[0].flatMap((v1) =>
                    attributeGroups[1].map((v2) => [v1.value, v2.value])
                  );
                } else {
          ...
          <Form.Item label="Regular Price">
            <Input
              placeholder="Enter Regular Price"
              value={multiRegularPrice}
              onChange={(e) => setMultiRegularPrice(e.target.value)}
            />
          </Form.Item>
          <Form.Item label="Offer Price">
            <Input
              placeholder="Enter Offer Price"
              value={multiOfferPrice}
              onChange={(e) => setMultiOfferPrice(e.target.value)}
            />
          </Form.Item>
          <Form.Item label="Current Stock">
            <Input
              ...
              value={multiStock}
              onChange={(e) => setMultiStock(e.target.value)}
            />
          </Form.Item>
          <Form.Item label="Is Default">
            <Select
              placeholder="Select Default"
              value={multiIsDefault}
              onChange={setMultiIsDefault}
              options={[
                { label: "No", value: 0 },
                { label: "Yes", value: 1 },
              ]}
