...
export default function EditCategory() {
    ...
    useEffect(() => {
        ...
        const getData = async () => {
            const res = await getDatas(`/admin/categories/${id}`);

            if(!isAlive) return;

            if(res?.success){
                const category = res?.result;
                const imageFileList = category.image
                    ? [
                        {
                            uid: "-1",
                            name: "category.png",
    ...
    const handleSubmit = async (values) => {
        ...
        try {
            const res = await postData(`/admin/categories/${id}`, formData);
        ...
        <>
            ...
            <div className="blog-form">
                <div className="form-head-wrapper">
                    <h2 className="form-head-title">Edit Category</h2>
