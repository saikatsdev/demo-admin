...
import { Breadcrumb, Button, Card, Col, Divider, Form, Input, InputNumber, message, Radio, Row, Select, Space, Tag, Typography, Upload,Input as AntInput } from "antd";
...
export default function AddCrossSell() {
    // Hook
    useTitle("Create Cross Sell Products");
    ...
    const handleSubmit = async (values) => {
        ...
        data.cross_sell_offers = selectedProducts.map((p) => ({
            product_id     : p.id,
            custom_name    : p.name,
        ...
        try {
            ...
            const res = await postData("/admin/cross-sells", data);
                                            ...
                                            filteredProducts?.map((product) => (
                                                <div key={product.id} className="search-item-row" onClick={() => handleSelectProduct(product, "main")}>
                                                    ...
                                                    <div className="info">
                                                        ...
                                                        <div className="meta">
                                                            {product.category_name ? product.category_name : "N/A"} | ৳{product.sell_price}
                                                        </div>
                                                    </div>
                    ...
                    <Col xs={24} lg={8}>
                        <Card title={<Space><ThunderboltOutlined /> Trigger Rules</Space>} className="modern-antd-card sticky-card">
                            ...
                            <Button type="primary" size="large" block loading={formLoading} htmlType="submit" className="submit-upsell-btn">
                                {formLoading ? "Creating..." : "Create Cross sell Offer"}
                            </Button>
                        </Card>
                    </Col>
                </Row>
