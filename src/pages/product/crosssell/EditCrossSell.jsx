...
export default function EditCrossSell() {
    // Hook
    useTitle("Edit Cross sell Offer");
    ...
    const [trigger, setTrigger]                                 = useState("all_product");
    ...
    const [offerInfo, setOfferInfo]                             = useState({ title: "", status: "active", started_at: "", ended_at: "", trigger_rules: "all_product" });
    ...
    useEffect(() => {
        ...
        const getOfferInfo = async () => {
            setLoading(true);
            try {
                const res = await getDatas(`/admin/cross-sells/${id}`);
                if (res && res?.success && isMounted) {
                    const data = res.result || {};
                    const formatDate = (dateTime) => dateTime ? dateTime.split(" ")[0] : "";
                    
                    const triggerRule = data.condition_type || "all_product";
    ...
    const handleSubmit = async (e) => {
        ...
        const data = {
            ...
            cross_sell_offers: selectedProducts.map(p => ({
                product_id     : p.id,
                custom_name    : p.name,
        ...
        try {
            setFormLoading(true);
            const res = await postData(`/admin/cross-sells/${id}`, data);
            if (res && res?.success) {
                messageApi.success(res.msg || "Cross sell offer updated successfully");
                setTimeout(() => navigate("/cross-sell"), 400);
            }
        } catch (error) {
            console.log(error);
        ...
        <div style={{ padding: 16 }}>
            ...
            <Spin spinning={loading && !selectedProducts.length}>
                <Form layout="vertical" onSubmitCapture={handleSubmit}>
                    <Row gutter={[20, 20]}>
                        <Col xs={24} lg={16}>
                            <Card title={<Space><SettingOutlined /> General Information</Space>} className="modern-antd-card" extra={<Button icon={<ArrowLeftOutlined />} onClick={() => navigate("/cross-sell")}>Back</Button>}>
                                <Row gutter={16}>
                                    <Col span={24}>
                            ...
                            <Card title={<Space><ThunderboltOutlined /> Trigger Rules</Space>} className="modern-antd-card sticky-card">
                                <Form.Item label="When should this offer show?">
                                    <Select size="large" value={trigger} onChange={handleTrigger} style={{ width: '100%' }}>
                                        <Select.Option value="all_product">For all Products</Select.Option>
