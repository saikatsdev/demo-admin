...
export default function EditReturnRefundPolicy() {
    ...
    useEffect(() => {
        const getPrivacy = async () => {
            const res = await getDatas(`/admin/return/policy/${id}`);
    ...
    const handleSubmit = async (values) => {
        ...
        try {
            setLoading(true);
            const res = await postData(`/admin/return/policy/${id}`, formData);
        ...
        <>
            ...
            <div className="pagehead">
                <div className="head-left">
                    <h1 className="title">Update Return & Refund Policy</h1>
                </div>
                <div className="head-actions">
                    <Breadcrumb
                        items={[
                            { title: <Link to="/dashboard">Dashboard</Link> },
                            { title: "Update Return & Refund Policy" },
                        ]}
                    />
                </div>
            </div>
