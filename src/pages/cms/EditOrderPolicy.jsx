...
export default function EditOrderPolicy() {
    // Hook
    useTitle("Edit Order Policy");
    ...
    useEffect(() => {
        const getPrivacy = async () => {
            const res = await getDatas(`/admin/order/policy/${id}`);
    ...
    const handleSubmit = async (values) => {
        ...
        try {
            setLoading(true);
            const res = await postData(`/admin/order/policy/${id}`, formData);
        ...
        <>
            ...
            <div className="pagehead">
                <div className="head-left">
                    <h1 className="title">Update Order Policy</h1>
                </div>
                <div className="head-actions">
                    <Breadcrumb
                        items={[
                            { title: <Link to="/dashboard">Dashboard</Link> },
                            { title: "Update Order Policy" },
                        ]}
                    />
                </div>
            </div>
