...
export default function EditWarrantyPolicy() {
    // Hook
    useTitle("Edit Warranty Policy");
    ...
    useEffect(() => {
        const getPrivacy = async () => {
            const res = await getDatas(`/admin/warranty/policy/${id}`);
    ...
    const handleSubmit = async (values) => {
        ...
        try {
            setLoading(true);
            const res = await postData(`/admin/warranty/policy/${id}`, formData);
        ...
        <>
            ...
            <div className="pagehead">
                <div className="head-left">
                    <h1 className="title">Update Warranty Policy</h1>
                </div>
                <div className="head-actions">
                    <Breadcrumb
                        items={[
                            { title: <Link to="/dashboard">Dashboard</Link> },
                            { title: "Update Warranty Policy" },
                        ]}
                    />
                </div>
            </div>
