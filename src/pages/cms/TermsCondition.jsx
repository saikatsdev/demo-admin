...
export default function TermsCondition() {
    ...
    const [termsCondition, setItems]      = useState([]);
    const [messageApi, contextHolder]     = message.useMessage();
    const [filteredData, setFilteredData] = useState(termsCondition);
    ...
    const onEdit = (record) => {
        navigate(`/edit/terms-condition/${record.id}`);
    }

    useEffect(() => {
        if(!query){
            setFilteredData(termsCondition);
        }

        const lowerQuery = query.toLowerCase();

        const filtered = termsCondition?.filter(item => 
    ...
    useEffect(() => {
        ...
        const fetchContactList = async () => {
            ...
            const res = await getDatas("/admin/terms-and-conditions");
    ...
    const onDelete = async (id) => {
        const res = await deleteData(`/admin/terms-and-conditions/${id}`);

        if(res?.success){
            const refreshed = await getDatas("/admin/terms-and-conditions");
        ...
        <>
            ...
            <div className="pagehead">
                <div className="head-left">
                    <h1 className="title">All Terms & Condition List</h1>
                </div>
                <div className="head-actions">
                    <Breadcrumb
                        items={[
                            { title: <Link to="/dashboard">Dashboard</Link> },
                            { title: "All Terms & Condition List" },
                        ]}
                    />
                </div>
            </div>
