...
export default function WarrantyPolicy() {
    ...
    //Method
    const openCreate = () => {
        navigate("/add/warranty/policy");
    }

    const onEdit = (record) => {
        navigate(`/edit/warranty/policy/${record.id}`);
    }
    ...
    useEffect(() => {
        ...
        const fetchContactList = async () => {
            ...
            const res = await getDatas("/admin/warranty/policy");
    ...
    const onDelete = async (id) => {
        const res = await deleteData(`/admin/warranty/policy/${id}`);

        if(res?.success){
            const refreshed = await getDatas("/admin/warranty/policy");
        ...
        <>
            ...
            <div className="pagehead">
                <div className="head-left">
                    <h1 className="title">Warranty Policy List</h1>
                </div>
                <div className="head-actions">
                    <Breadcrumb
                        items={[
                            { title: <Link to="/dashboard">Dashboard</Link> },
                            { title: "Warranty Policy List" },
                        ]}
                    />
                </div>
            </div>
