...
export default function PrivacyPolicy() {
    ...
    const onEdit = (record) => {
        navigate(`/edit/privacy/${record.id}`);
    }
    ...
    useEffect(() => {
        ...
        const fetchContactList = async () => {
            ...
            const res = await getDatas("/admin/privacy-policies");
    ...
    const onDelete = async (id) => {
        const res = await deleteData(`/admin/privacy-policies/${id}`);

        if(res?.success){
            const refreshed = await getDatas("/admin/privacy-policies");
        ...
        <>
            ...
            <div className="pagehead">
                <div className="head-left">
                    <h1 className="title">All Privacy List</h1>
                </div>
                <div className="head-actions">
                    <Breadcrumb
                        items={[
                            { title: <Link to="/dashboard">Dashboard</Link> },
                            { title: "All Privacy List" },
                        ]}
                    />
                </div>
            </div>
