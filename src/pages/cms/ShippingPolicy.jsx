...
export default function ShippingPolicy() {
    //Hook
    useTitle("Shipping Policy | Service Key");
    ...
    //Method
    const openCreate = () => {
        navigate("/add/shipping/policy");
    }

    const onEdit = (record) => {
        navigate(`/edit/shipping/policy/${record.id}`);
    }
    ...
    useEffect(() => {
        ...
        const fetchContactList = async () => {
            ...
            const res = await getDatas("/admin/shipping/policy");
    ...
    const onDelete = async (id) => {
        const res = await deleteData(`/admin/shipping/policy/${id}`);

        if(res?.success){
            const refreshed = await getDatas("/admin/shipping/policy");
        ...
        <>
            ...
            <div className="pagehead">
                <div className="head-left">
                    <h1 className="title">Shipping Policy List</h1>
                </div>
                <div className="head-actions">
                    <Breadcrumb
                        items={[
                            { title: <Link to="/dashboard">Dashboard</Link> },
                            { title: "Shipping Policy List" },
                        ]}
                    />
                </div>
            </div>
