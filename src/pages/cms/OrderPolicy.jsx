...
export default function OrderPolicy() {
    ...
    //Method
    const openCreate = () => {
        navigate("/add/order/policy");
    }

    const onEdit = (record) => {
        navigate(`/edit/order/policy/${record.id}`);
    }
    ...
    useEffect(() => {
        ...
        const fetchContactList = async () => {
            ...
            const res = await getDatas("/admin/order/policy");
    ...
    const onDelete = async (id) => {
        const res = await deleteData(`/admin/order/policy/${id}`);

        if(res?.success){
            const refreshed = await getDatas("/admin/order/policy");
        ...
        <>
            ...
            <div className="pagehead">
                <div className="head-left">
                    <h1 className="title">Order Policy List</h1>
                </div>
                <div className="head-actions">
                    <Breadcrumb
                        items={[
                            { title: <Link to="/dashboard">Dashboard</Link> },
                            { title: "Order Policy List" },
                        ]}
                    />
                </div>
            </div>
