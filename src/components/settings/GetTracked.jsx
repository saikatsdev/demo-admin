...
export default function GetTracked({formatText}) {
    // State
    ...
    useEffect(() => {
        ...
        const fetcheDashboard = async () => {
            const res = await getDatas("/admin/settings", { setting_category_id: 9 });
    ...
    const dashboardSettings = dashboardData.filter((item) => item.category?.id == 9);
    ...
    return (
        <div style={{ padding: '24px 0', maxWidth: '1200px', margin: '0 auto' }}>
            ...
            <div style={{ marginBottom: '24px' }}>
                <Title level={3} style={{ margin: 0, color: '#1f2937' }}>GetTracked Settings</Title>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                {dashboardSettings.map((product) => product.key && (
                    <Card 
                        key={product.id} 
                        hoverable 
                        style={{ 
                            ...
                            <Col xs={24} md={7}>
                                <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
                                    ...
                                    <Text type="secondary" style={{ marginTop: '8px', fontSize: '14px', lineHeight: '1.5' }}>
                                        {product.instruction || 'Configure this dashboard setting for your administrative panel.'}
                                    </Text>
                                </div>
                            </Col>
